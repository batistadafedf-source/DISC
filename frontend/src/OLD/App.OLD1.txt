...
import ChartDataLabels from "chartjs-plugin-datalabels"; // Plugin para exibir números nos gráficos
...
const FormularioDISC = () => {
  ...
  const categorias = ["D", "I", "S", "C"];
   ...
   //Lista das opções para o usuário preencher
    const quadros = [
     [
  ...
  const handleChange = (quadroIndex, id, tipo, categoria) => {
    setRespostas((prev) => ({
      ...prev,
      [quadroIndex]: {
        ...prev[quadroIndex],
  ...
  const handleSubmit = async (e) => {
    ...
    // Contagem dos valores para os gráficos
  ...
  const opcoesGrafico = {
    ...
    scales: {
      y: { min: -22, max: 30, ticks: { stepSize: 2 } },
    },
  };
    ...
    <div style={{ textAlign: "center" }}>
            ...
            <table border="1" cellPadding="10" style={{ margin: "auto", width: "70%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  ...
                  <th style={{ width: "20%" }}>Mais se parece</th>
                  <th style={{ width: "20%" }}>Menos se parece</th>
                </tr>
              </thead>
              <tbody>
                {quadro.map((pergunta) => (
      ...
      {dadosGraficoExterno && dadosGraficoInterno && dadosGraficoSintese && (
        <div style={{ width: "50%", margin: "auto", marginTop: "30px" }}>
          ...
          <Line data={dadosGraficoExterno} options={{ ...opcoesGrafico, plugins: { ...opcoesGrafico.plugins, title: { text: "Externo (MAIOR)" } } }} />
          <Line data={dadosGraficoInterno} options={{ ...opcoesGrafico, plugins: { ...opcoesGrafico.plugins, title: { text: "Interno (MENOR)" } } }} />
          <Line data={dadosGraficoSintese} options={{ ...opcoesGrafico, plugins: { ...opcoesGrafico.plugins, title: { text: "Síntese" } } }} />
        </div>
      )}
    </div>
  );
